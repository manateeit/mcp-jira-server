...
async function main() {
  try {
    ...
    if (!connectionOk) {
      ...
      console.error(
        '3. JIRA_API_TOKEN is valid (create at https://id.atlassian.com/manage-profile/security/api-tokens)',
      );
    ...
    console.error(
      `Tools: ${Object.keys(tools).length}, Prompts: ${Object.keys(prompts).length}, Resources: ${Object.keys(resources).length}`,
    );
  } catch (error) {
