...
import { EnhancedJqlService } from '../jira/service.js';
import { JiraClientConfig, SearchRequest } from '../jira/types.js';
...
const CreateIssueSchema = z.object({
  ...
  issueType: z
    .string()
    .describe(
      'Type of issue (e.g., "Bug", "Task", "Story", "Epic", "Subtask", "Úkol", "Dílčí úkol")',
    ),
  assignee: z.string().optional().describe('Email or account ID of assignee'),
  priority: z
    .string()
    .optional()
    .describe('Priority (e.g., "Highest", "High", "Medium", "Low", "Lowest", or localized names)'),
  ...
  dueDate: z
    .string()
    .optional()
    .describe('Due date (formats: "2024-12-31", "31.12.2024", "tomorrow", "next week", "+7d")'),
  startDate: z.string().optional().describe('Start date (same formats as dueDate)'),
  originalEstimate: z
    .string()
    .optional()
    .describe('Original time estimate (e.g., "2h", "1d", "3d 4h")'),
...
const SearchIssuesSchema = z.object({
  ...
  dueBefore: z
    .string()
    .optional()
    .describe('Filter by due date before (e.g., "2024-12-31", "next week")'),
  dueAfter: z
    .string()
    .optional()
    .describe('Filter by due date after (e.g., "2024-01-01", "today")'),
  ...
  // Enhanced JQL API parameters
  fields: z.union([
    z.array(z.string()),
    z.enum(['*all', '*navigable']),
  ]).optional().describe('Fields to include in response (array of field names or "*all", "*navigable")'),
  expand: z.array(z.string()).optional().describe('Additional data to expand (e.g., "changelog", "renderedFields")'),
  cursor: z.string().optional().describe('Pagination cursor token from previous response'),
  method: z.enum(['auto', 'GET', 'POST']).optional().default('auto').describe('HTTP method selection strategy'),
  
  // Legacy parameters for backward compatibility (deprecated)
  limit: z.number().optional().describe('DEPRECATED: Use maxResults instead. Maps to maxResults parameter'),
  startAt: z.number().optional().describe('DEPRECATED: Use cursor-based pagination instead. Converted to cursor navigation when possible'),
});
...
const GetCommentsSchema = z.object({
  ...
  orderBy: z
    ...
    .optional()
    .default('-created')
    .describe('Sort order (newest first by default)'),
});
...
const CreateTaskForEpicSchema = z.object({
  ...
  issueType: z
    ...
    .optional()
    .default('Úkol')
    .describe('Issue type (defaults to "Úkol" for Czech Jira)'),
});
...
const CreateSprintSchema = z.object({
  ...
  startDate: z
    .string()
    .optional()
    .describe('Start date (formats: "2024-12-31", "31.12.2024", "today", "tomorrow")'),
  endDate: z
    .string()
    .optional()
    .describe('End date (formats: "2024-12-31", "31.12.2024", "next week", "+14d")'),
});
...
const BatchCommentSchema = z.object({
  ...
  continueOnError: z
    ...
    .optional()
    .default(true)
    .describe('Continue processing if one comment fails'),
});
...
/**
 * Create Enhanced JQL service from existing config
 */
function createEnhancedJqlService(config: Config): EnhancedJqlService {
  // Extract domain from jiraHost URL
  const hostUrl = new URL(config.jiraHost);
  const domain = hostUrl.hostname.replace('.atlassian.net', '');
  
  const jiraClientConfig: JiraClientConfig = {
    domain: domain,
    email: config.jiraEmail,
    apiToken: config.jiraApiToken,
    timeout: 30000,
    userAgent: 'MCP-Jira-Server-Enhanced/2.0.0',
    enableLogging: true,
  };
  
  logger.info('Creating Enhanced JQL service with configuration', {
    domain: domain,
    email: config.jiraEmail,
    userAgent: jiraClientConfig.userAgent,
  });
  
  return new EnhancedJqlService(jiraClientConfig);
}
  ...
  return {
      ...
      handler: async (args: unknown) => {
        try {
          ...
          if (params.acceptanceCriteria && customFields.acceptanceCriteria) {
            ...
            try {
              payload.fields[customFields.acceptanceCriteria] = textToADF(
          ...
          if (
            (params.createTestTicket ?? config.autoCreateTestTickets) &&
            normalizeIssueType(params.issueType) === 'Story'
          ) {
            try {
    ...
    'update-issue': {
      ...
      handler: async (args: unknown) => {
        ...
        if (params.originalEstimate || params.remainingEstimate) {
          payload.fields.timetracking = {};
          if (params.originalEstimate) {
            payload.fields.timetracking.originalEstimate = parseTimeEstimate(
              params.originalEstimate,
            );
          }
          if (params.remainingEstimate) {
            payload.fields.timetracking.remainingEstimate = parseTimeEstimate(
              params.remainingEstimate,
            );
          }
        }
    ...
    'search-issues': {
      ...
      handler: async (args: unknown) => {
        ...
        // Apply backward compatibility mapping for legacy parameters
        let maxResults = params.maxResults;
        
        // Map legacy 'limit' parameter to 'maxResults'
        if (params.limit !== undefined) {
          logger.debug('Mapping legacy "limit" parameter to "maxResults"', { 
            limit: params.limit, 
        ...
        // Log deprecation warnings for legacy parameters
        if (params.limit !== undefined) {
          logger.warn('Parameter "limit" is deprecated. Please use "maxResults" instead for Enhanced JQL API compatibility.');
        }
        if (params.startAt !== undefined) {
          logger.warn('Parameter "startAt" is deprecated. Please use cursor-based pagination with "cursor" parameter instead for better performance.');
          // Note: startAt conversion to cursor would require complex state management
          // For now, we log the warning but still use maxResults-based pagination
          if (params.startAt > 0) {
            logger.warn(`startAt offset (${params.startAt}) cannot be directly converted to cursor pagination. Consider using cursor-based pagination for large datasets.`);
          }
        }
        ...
        if (!jql) {
          ...
          if (params.assignee) {
            const assigneeValue =
              params.assignee === 'currentUser' ? 'currentUser()' : `"${params.assignee}"`;
        ...
        try {
          // Create Enhanced JQL service for this search
          const enhancedJqlService = createEnhancedJqlService(config);
          
          // Build Enhanced JQL search request with backward compatibility
          const searchRequest: SearchRequest = {
            ...
            // Legacy parameter support (already mapped above)
            limit: params.limit,
            startAt: params.startAt,
          };
          
          // Log MCP tool search operation with structured data
          logger.logSearchOperation({
            operationId: `mcp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
            operationType: params.cursor ? 'cursor-navigation' : 'search',
            jql: jql,
            jqlLength: jql.length,
            maxResults,
            cursor: params.cursor,
            httpMethod: params.method === 'auto' ? undefined : params.method,
            timestamp: Date.now(),
            projectKey: params.project,
            domain: config.jiraHost ? new URL(config.jiraHost).hostname.replace('.atlassian.net', '') : undefined,
          });
          
          // Execute Enhanced JQL search
          const response = await enhancedJqlService.search(searchRequest);
          
          // Format response using Enhanced JQL formatter
          return {
            content: [
              {
                type: 'text',
                text: JiraFormatter.formatEnhancedSearchResults(response),
              },
            ],
          };
        } catch (error: any) {
          logger.error('Enhanced JQL search failed', { error: error.message, jql });
          
          // Enhanced error handling for Jira API errors
          if (error.errorMessages && Array.isArray(error.errorMessages)) {
            const errorMessage = error.errorMessages.join(', ');
            return {
              content: [
                {
                  type: 'text',
      ...
      handler: async (args: unknown) => {
        try {
          ...
          if (
            ...
            !Array.isArray(transitionsResult.data.transitions)
          ) {
    ...
    'link-issues': {
      ...
      handler: async (args: unknown) => {
        try {
          ...
          if (
            !result.success &&
            (result.error?.includes('typ odkazu') || result.error?.includes('link type'))
          ) {
            ...
            if (linkTypesResult.success && linkTypesResult.data) {
              ...
              if (Array.isArray(linkTypes) && linkTypes.length > 0) {
                ...
                const matchingType = linkTypes.find((lt: any) => {
                  return (
                    lt.name?.toLowerCase() === requestedType ||
                ...
                logger.warn(
                  'No link types found or invalid response structure',
                  linkTypesResult.data,
                );
              }
            }
      ...
      handler: async (args: unknown) => {
        try {
          ...
          for (const subtask of params.subtasks) {
            try {
              ...
              if (subtaskResult.success && subtaskResult.data?.key) {
                ...
                failedSubtasks.push(
                  `❌ "${subtask.summary}": ${subtaskResult.error || 'Unknown error'}`,
                );
              }
            } catch (error) {
              logger.error('Error creating subtask', { summary: subtask.summary, error });
              failedSubtasks.push(
                `❌ "${subtask.summary}": ${error instanceof Error ? error.message : 'Unknown error'}`,
              );
            }
          }
          ...
          if (failedSubtasks.length > 0) {
            ...
            response +=
              '⚠️ Some subtasks failed (Jira might not support subtasks under epics). Creating tasks instead:\n\n';
    ...
    'get-fields': {
      ...
      handler: async (args: unknown) => {
        try {
          ...
          let issueType;
          if (params.issueType) {
            issueType = project.issuetypes?.find(
              (it: any) => it.name.toLowerCase() === params.issueType?.toLowerCase(),
            );
            if (!issueType) {
    ...
    'get-history': {
      ...
      handler: async (args: unknown) => {
        try {
          ...
          const formattedHistory = histories
            .map((history: any) => {
    ...
    'upload-attachment': {
      ...
      handler: async (args: unknown) => {
        try {
          ...
          logger.debug('Uploading attachment to issue', {
            issueKey: params.issueKey,
    ...
    'get-boards': {
      ...
      handler: async (args: unknown) => {
        try {
          ...
          const formattedBoards = filteredBoards
            .map((board: any) => {
        ...
        try {
          ...
          if (successful.length > 0) {
            ...
            responseText += successful
              .slice(0, 5)
              .map(r => `• ${JiraFormatter.formatIssueLink(r.issueKey, config.jiraHost)}`)
              .join('\n');
